...
typedef RT_MODEL_MPC_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * MPC(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(MPC_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  for (count = 1; count < argc; ) {
    ...
    } else if ((strcmp(option, "-td") == 0) && (count != argc)) {/* delay time (delays initialization of model) */
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://MPC:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: MPC -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
